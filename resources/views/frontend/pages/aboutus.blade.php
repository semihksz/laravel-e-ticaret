@extends('frontend.layout.layout')

@section('content')
    <div class="bg-light py-3">
        <div class="container">
            <div class="row">
                <div class="col-md-12 mb-0"><a href="{{ route('index') }}">Home</a> <span class="mx-2 mb-0">/</span> <strong
                        class="text-black">About</strong></div>
            </div>
        </div>
    </div>

    <div class="site-section border-bottom" data-aos="fade">
        <div class="container">
            <div class="row mb-5">
                <div class="col-md-6">
                    <div class="block-16">
                        <figure>
                            <img src="images/{{ $abouts->image }}" alt="Image placeholder" class="img-fluid rounded">
                            <a href="https://vimeo.com/channels/staffpicks/93951774" class="play-button popup-vimeo"><span
                                    class="ion-md-play"></span></a>

                        </figure>
                    </div>
                </div>
                <div class="col-md-1"></div>
                <div class="col-md-5">


                    <div class="site-section-heading pt-3 mb-4">
                        <h2 class="text-black">{{ $abouts->title }}</h2>
                    </div>
                    <p>{{ $abouts->content }}</p>

                </div>
            </div>
        </div>
    </div>

    <div class="site-section border-bottom" data-aos="fade">
        <div class="container">
            <div class="row justify-content-center mb-5">
                <div class="col-md-7 site-section-heading text-center pt-4">
                    <h2>The Team</h2>
                </div>
            </div>
            <div class="col-md-12 col-lg-3">
                <div class="row">
                    <div class="block-38 text-center">
                        <div class="block-38-img">
                            <div class="block-38-header">
                                <img src="images/{{ $abouts->personImage }}" alt="Image placeholder" class="mb-4">
                                <h3 class="block-38-heading h4">{{ $abouts->personName }}</h3>
                                <p class="block-38-subheading">{{ $abouts->personPosition }}</p>
                            </div>
                            <div class="block-38-body">
                                <p>{{ $abouts->personDescription }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>


    <div class="site-section site-section-sm site-blocks-1 border-0" data-aos="fade">
        <div class="container">
            <div class="row">
                <div class="col-md-6 col-lg-4 d-lg-flex mb-4 mb-lg-0 pl-4" data-aos="fade-up" data-aos-delay="">
                    <div class="icon mr-4 align-self-start">
                        <span class="{{ $abouts->textIcon1 }}"></span>
                    </div>
                    <div class="text">
                        <h2 class="text-uppercase">{{ $abouts->textTitle1 }}</h2>
                        <p>{{ $abouts->textDescription1 }}</p>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4 d-lg-flex mb-4 mb-lg-0 pl-4" data-aos="fade-up" data-aos-delay="100">
                    <div class="icon mr-4 align-self-start">
                        <span class="{{ $abouts->textIcon2 }}"></span>
                    </div>
                    <div class="text">
                        <h2 class="text-uppercase">{{ $abouts->textTitle2 }}</h2>
                        <p>{{ $abouts->textDescription2 }}</p>
                    </div>
                </div>
                <div class="col-md-6 col-lg-4 d-lg-flex mb-4 mb-lg-0 pl-4" data-aos="fade-up" data-aos-delay="200">
                    <div class="icon mr-4 align-self-start">
                        <span class="{{ $abouts->textIcon3 }}"></span>
                    </div>
                    <div class="text">
                        <h2 class="text-uppercase">{{ $abouts->textTitle3 }}</h2>
                        <p>{{ $abouts->textDescription3 }}</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
